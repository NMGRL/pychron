...
5    open(unit = 31, file = 'files.cl', status = 'old')
...
!	a4 = 36Ar/39Ar
!	a5 = mol 39Ar
!	f39(n) = fraction of 39Ar
!	a6 = % 40Ar*
!	a7 = 40Ar*/39Ark
!	age(n)= Apparent age in yr
!	sig(n)= sigma uncertainty of age in yr
!	time(n) = time in minutes
!	a8 = 39Ar/40Ar
!	a9 = sigma uncertainty 39Ar/40Ar
!	a10= 36Ar/40Ar
!	a11= sigma uncertainty of 36Ar/40Ar
!	Tinv= Inverse of temperature
!	a13= sigma  uncertainty of Temperature
!	xlog= log(D/r**2)
!	a15= sigma uncertainty of log(D/r**2)
!	SUBROUTINE ARR.F
! **********************************************************************
...
subroutine param(tinv, wt, e, ord, yes)
    ...
    common /var/ f(0:ns), telab(ns), tilab(ns), xlogd(0:ns), sig39(ns), &
            xlogr(0:ns), a39(ns), acut, b, sige, sigo, qmax, chisqe
    common /int/ ni, imp, nimax, ke
    character tab1*9, yes*1, yesp*1, yestemp*1
    tab1 = char(09)
    nst = nstop
    y(3) = 0.
    if(ni.lt.nstop) then
...
subroutine errcal(wt, swt)
    ...
    sigzit(0) = 0.
    do 10 i = 1, ni
        !         x=dlog10(sig39(i))+15.d0
        !        siga0=a0+a1*x+a2*x**2+a3*x**3
        !        siga0 = 10**(siga0-2.d0)
        !	   siga0 =0.
    ...
    do 30 i = 1, ni
        if (f(i).le.acut) then
            ...
            !            as1= (sigsm(i)/(an1*(1-f(i-1))))**2/dzit2
