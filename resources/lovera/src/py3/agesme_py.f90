!	PROGRAM TO CALCULATE AN AGE SPECTRUM FROM A GIVEN THERMAL
!		HISTORY. (AGES.F) (SPHERES) 

!	R= GAS CONSTANT KCAL/(MOL*K)
!	XLAMBD= DECAY CONSTANT OF POTASSIUM IN 1/MYRS.
!	NUSA=# OF SAMPLES,  TOTIME = TOTAL TIME OF Ar ACCUMULATION
!	C40WD= TOTAL ARGON RADIOGENIC 40Ar PRODUCED POR UNIT VOL. FROM TIME=0.
!	C39WD= FRACTION OF 39Ar AT PRESENT.
!	NEND=TIME AT WHICH THE SYSTEM IS EFFECTIVELY CLOSED, TEMPERATURE
!       IS ASSUME CONSTANT FROM THIS TIME UNTIL PRESENT.
!	STEP= DURATION OF THE TIME STEP, PEND AND TEMIN DEFINE
!	 THE DIFFERENT SEGMENTS OF THE COOLING HISTORY
!	NI=# OF HEATING STEPS
!	TELAB = TEMPERATURE STEP (*K), TILAB= DURATION OF THE HEATING STEP (MIN)
!	NSAMP=# OF DIFFERENT DIFFUSION DOMAIN SIZES
!	D0(J)= FREQUENCY FACTOR (1/SEC) 
!	E(J)= ACTIVACTION ENERGY (KCAL/MOL)
!       C(J)= VOLUME FRACION
!	SUBROUTINE GEOSEC CALCULATE THE DISTRIBUTION OF 40Ar AT PRESENT WHICH
!	IS GIVEN BY THE COEFFICIENT XI(N).
!	SUBROUTINE LAB CALCULATE THE RELEASED OF 40Ar and 39Ar AT LAB. 
!	FOR EACH DIFFUSION DOMAIN SIZE. (REA40 AND REA39)
!	CURA= CUMULATIVE RATE OF A40 T0 A39 RELEASE.
!	RATE= STEP RATE OF A40 T0 A39 RELEASE
!	DR39 AND DR40 ARE THE STEP RELEASE OF A39 AND A40 RESPECTIVELY.
...
open(unit = 21, file = 'agesme.cl', status = 'old')

lc(0) = 0.
!	CALCULATION OF THE COOLING HISTORY
...
!  DEFINITION OF STEP HEATING SCHEDULED AT LAB.
...
!	CALCULATION OF THE AGE SPECTRUM

do 16 je = 1, nsamp
    if(imod.eq.1)d0(je) = d0(je) / 4.d0


    !	tau=r*550.**2/(e(1)*dtdt)
    !      tc= e(1)/r/dlog(8.7*tau*d0(1))-273.

    call geosec(ns, xlambd, d0(je), E(je), temp, time)
    call lab(d0(je), E(je), r40, r39, telab, tilab, dr39, ni, tab1)

    !	SUMMATION OF THE CONTRIBUTION OF EACH DIFFUSION DOMAIN SIZE.
...
subroutine geosec(nend, xlambd, d0, E, temp, time)
    ...
    !     COMPUTO OF XI(M) - M<80000 --> nn=340
    do 30 mi = 1, nn - 1
        xlogm = dlog(an(mi))
        sum = 0.d0
        do 40 n = 1, nend
...
!	SUBROUTINE TO CALCULATE THE THERMAL HISTORY.
!		 (SUBROUTINE SUBCHIST.F)
...
subroutine chist(np, tmin, tpin, time, temp, dt, ns, nch)
    ...
    !	DEFINITION OF THE TEMPERATURE STEP IN CENTIDEGREE.
