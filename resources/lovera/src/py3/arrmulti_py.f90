!	NEWARR2.F
!       ERROR ON BOTH AXIS  - USES OLD RAMDOM ROUTINE GUESS
!	TO USE NEW RAMDOM ROUTINE GUESS0 UNCOMMENT THE CALL GUESS0
!	AND COMMENT OUT THE CALL TO GUESS
!	FROM AUTOTEST DIR
! **********************************************************************
...
!         ARR.DAT   (*.ar1)       16              ARR
!         LOGR.DAT  (*.lg1)       18              ARR
!         ARR.SAMP  (*.ar0)       20              DIFF
!         LOGR.SAMP (*.lg0)       22              DIFF
!         SIZES.DAT               24              MAIN
!         VOLCON.DAT              26              MAIN
!         PARAM.OUT (*.par)       28              MAIN
!         ARR-ME.IN (*.ame)       30              MAIN
!         DIST.DAT  (*.dst)       32              MAIN
!         ENER.DAT                34              PARAM
!         INFO.DAT                36              MAIN
...
if(mct.gt.30) then
    write(28, *) 'Warning: Too many iterations. Consult your computer vodoo', sname(1:kn), ncicle
...
! ENTER HERE CODE FOR GAUSSIAN E DISTRIBUTION
...
Subroutine funcs(x, b, y, dyda, na, a, amax)
    ...
    !  THE MULTIPLICATION BY 4 STAND BY THE DIVITION OF D0
    if(na.eq.0)return
    y = 0.
    as = 1.
    do 5 j = 1, na, 2
...
subroutine mrqmin(x, y, sig, ndata, a, ma, lista, mfit, &
    ...
    if(amax.eq.-1.)return
    do j = 1, mfit, 2
        if(dabs(da(j)).gt.8)da(j) = sign(real*8, da(j))
        if(dabs(da(j + 1)).gt.3)da(j + 1) = sign(real*3, da(j + 1))
    enddo
    if(alamda.eq.0.)then
    ...
    do 16 j = 1, mfit
        ...
        if(j / 2 * 2 - j.ne.0.and.dabs(atry(lista(j))).gt.14)then
            atry(lista(j)) = sign(real*14, atry(lista(j))) + ran1(iseed)
        endif
    16    continue
    if(ma.ne.mfit) then
        do 22 k = 1, mfit - 1, 2
...
subroutine errcal(wt, swt)
    ...
    common /var/ f(0:ns), telab(ns), tilab(ns), xlogd(0:ns), sig39(ns), &
            xlogr(0:ns), a39(ns), acut, b, sige, sigo, qmax, chisqe
    common /int/ ni, imp, nimax, ke
    ! CALCULATION OF SIGMA F
    ! TO GENERALIZE TO HAVE POINTS WITH DIFFERENT % ERRORS
    ! READ SIGA FROM SOME FILE.
...
!     Compute samples from normal distribution with mean of 0 and
!       standard deviation of 1; based on the Box-Muller Transformation.
