...
from pychron.spectrometer.tests.mftable import Argon2CDDMFTableTestCase

# loader = unittest.TestLoader()
# suite = unittest.TestSuite()
# tests = (Argon2CDDMFTableTestCase,)
# for t in tests:
...
mftable = MagnetFieldTable(bind=False)
mftable.molweights = {'Ar40': 40, 'Ar39': 39, 'Ar36': 36, 'Ar38': 38, 'PHHCbs': 1}

p = './data/argon_2CDD.csv'
if not os.path.isfile(p):
    p = 'spectrometer/tests/data/argon_2CDD.csv'

mftable._test_path = p
mftable.load_mftable(path=p)


mftable.update_field_table('L2(CDD)', 'Ar36', 3.76439824048, report=True, save=False)
mftable.update_field_table('H2', 'Ar40', 3.76429804008, report=True, save=False)

print mftable.get_dac('AX(CDD)', 38)
