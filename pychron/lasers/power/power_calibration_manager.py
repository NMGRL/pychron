...
class PowerCalibrationManager(Manager):
    ...
    def _graph_factory(self):
        ...
        g = Graph(
            window_title="{} Power Calibration {}".format(name, cnt),
    ...
    def _execute_power_calibration(self):
        ...
        _dn = dm.new_frame(directory=directory, base_frame_name="power_calibration")

        table = dm.new_table("/", "calibration", table_style="PowerCalibration")
        callback = lambda p, r, t: self._write_data(p, r, t)
        self._stop_signal = TEvent()
        self._iterate(self.parameters, self.graph, True, callback, table)
    ...
    def _iterate(self, params, graph, is_calibrating, callback, *args):
        pstop = params.pstop
        pstep = params.pstep
    ...
    def kill(self):
        ...
        if self.initialized:
            for n in ["parameters", "check_parameters"]:
                with open(self._get_parameters_path(n), "wb") as f:
        ...
        v = View(
            VGroup(
                ...
                VGroup(
                    Item(
                        "coefficients",
                        tooltip="Polynomial coefficients. Enter A,B,C... where\
    ...
    def _get_coefficients(self):
        return (
            ",".join(["{:0.4f}".format(c) for c in self._coefficients])
    ...
    def dump_power_calibration(self, coefficients, calibration_path=None):
        ...
        coeffstr = lambda c: "calibration coefficients= {}".format(
            ", ".join(map("{:0.3e}".format, c))
        )
...
class FusionsCO2PowerCalibrationManager(PowerCalibrationManager):
    ...
    def _apply_fit(self, new=True):
        ...
        g = self.graph
        if new:
            s, p = g.new_series(x, y, color="red", aux_plot=True, dash="dash")
    ...
    def _get_iteration_data(self, pi):
        ...
        if apm is not None:
            ...
            for _ in range(nintegrations):
                ...
                time.sleep(max(1e-6, integration_period - (gtime() - st)))
