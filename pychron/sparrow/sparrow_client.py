...
def datum_factory(p):
    return {'type': {'parameter': p['kind'], 'unit': p.get('unit', '')},
            'value': p['value'],
            'error': p['error']}


def analyses_payload(ans, preferred_kinds):
    analyses = [{'is_bad': ai.get('tag', 'ok').lower() == 'invalid',
                 'analysis_type': 'Step Heat' if is_step_heat(ai.get('record_id')) else 'Fusion',
                 'analysis_name': ai.get('record_id'),
                 'uuid': ai.get('uuid'),
                 'datum': [{'type': {'parameter': 'age', 'unit': ai.get('age_units', 'Ma')},
                            'value': ai.get('age', 0),
    ...
    preferreds = [{'analysis_name': 'preferred',
...
class SparrowClient(Loggable):
    ...
    def login(self):
        ...
        res = requests.post(url, json={"username": self.username, "password": self.password}, timeout=3)
        try:
            token = res.json()['token']
            self.token = token
            return True
        except KeyError:
            self.warning_dialog('failed connecting to Sparrow. Check connection and username/password')
    ...
    def insert_ia(self, path):
        ...
        # print(cm)
        session = {'sample': sample_payload(sm),
...
if __name__ == '__main__':
    s = SparrowClient(
        host='http://129.138.12.35', bind=False)
