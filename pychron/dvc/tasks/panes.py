    ...
    def traits_view(self):
        ...
        bookmark_grp = VGroup(
            BorderVGroup(
                UItem(
                    "git_tags",
                    editor=TabularEditor(adapter=GitTagAdapter()),
                    height=200,
                ),
                label="Bookmarks",
            )
            ...
            HGroup(
                Tabbed(
                    UItem(
                        "diffs",
                        label="Diffs",
                        editor=TabularEditor(adapter=DiffsAdapter(), editable=False),
                    ),
                    UItem("diff_text", label="Text", style="custom"),
                ),
                UItem(
                    "files",
                    style="custom",
                    editor=ListStrEditor(selected="selected_file", editable=False),
                ),
            ),
            label="Files",
        )
        a_grp = VGroup(
            UItem("analyses", style="custom", editor=ListStrEditor(editable=False)),
            label="Analyses",
        )
...
class SelectionPane(TraitsDockPane):
    ...
    def traits_view(self):
        ...
        local_grp = BorderVGroup(
            UItem(
                "filter_repository_value",
