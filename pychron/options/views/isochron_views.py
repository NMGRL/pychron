...
class InverseIsochronMainOptions(TitleSubOptions):
    def traits_view(self):
        info_grp = BorderHGroup(
            ...
            BorderHGroup(
                UItem("info_fontname"), UItem("info_fontsize"), enabled_when="show_info"
            ),
            label="Info",
        )
        ...
        agrp = HGroup(
            BorderHGroup(
                ...
                Item(
                    "results_info_spacing",
                    editor=RangeEditor(mode="spinner", low=2, high=20, is_float=False),
                    label="Spacing",
                ),
            ),
            BorderHGroup(UItem("results_fontname"), UItem("results_fontsize")),
        )
        bgrp = HGroup(
            BorderHGroup(Item("include_sample", label="Sample/Identifer")),
            BorderHGroup(
                Item("include_4036_mse", label="MSE"),
                Item("include_percent_error", label="%Err."),
                Item("yintercept_sig_figs", label="SigFigs"),
                label="Ar40/Ar36",
            ),
            BorderHGroup(
                Item("include_age_mse", label="MSE"),
        ...
        error_display_grp = BorderHGroup(
            Item("fill_ellipses", label="Fill"),
            Item("ellipse_kind", label="Kind"),
            Item("include_error_envelope"),
            label="Error Display",
        )
        label_grp = BorderVGroup(
            Item("show_labels", label="Display Labels"),
            HGroup(
                Item("label_box"),
...
class InverseIsochronInset(SubOptions):
    def traits_view(self):
        xbounds = BorderHGroup(
            Item("inset_xmin", label="Min."),
            Item("inset_xmax", label="Max."),
            label="X Limits",
        )
        ybounds = BorderHGroup(
            Item("inset_ymin", label="Min."),
            Item("inset_ymax", label="Max."),
            label="Y Limits",
        )
        e = BorderHGroup(
            Item("inset_show_error_ellipse", label="Show"),
        ...
        g = VGroup(
            ...
            HGroup(
                Item("inset_width", label="Width"), Item("inset_height", label="Height")
            ),
            HGroup(
                Item("inset_label_fontname", label="Label Font"),
                UItem("inset_label_fontsize"),
            ),
            HGroup(
                Item(
                    "inset_link_status",
                    label="Link Omit Status",
...
ISOCHRON_VIEWS = {
    MAIN.lower(): IsochronMainOptions,
    APPEARANCE.lower(): IsochronAppearance,
}

INVERSE_ISOCHRON_VIEWS = {
    MAIN.lower(): InverseIsochronMainOptions,
