...
from pychron.pipeline.tagging.data_reduction_tags import (
    DataReductionTagModel,
    SelectDataReductionTagModel,
)
...
class ItemAdapter(TabularAdapter):
    columns = [("Run ID", "record_id"), ("Sample", "sample"), ("Tag", "tag")]
    font = "arial 10"
...
class AnalysisTagView(Controller):
    ...
    def traits_view(self):
        note_grp = VGroup(UItem("note", style="custom"), show_border=True, label="Note")
        ...
        v = okcancel_view(
            VGroup(
                ...
                HGroup(
                    Item("use_filter", label='Remove "Invalid" analyses from figure'),
                    defined_when="items",
                ),
            ),
            resizable=True,
