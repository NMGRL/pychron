        ...
        if recaller.connect():
            ...
            with recaller.db.session_ctx():
                for level in state.levels:
                    ...
                    dvc.add_irradiation_level(
                        level, new_irradiation, dblevel.SampleHolder, "PlaceHolder"
                    )
                for level in state.levels:
                    positions = []
                    for ip in recaller.db.get_irradiation_positions(irradiation, level):
                        positions.append(
                            {
                                "identifier": str(ip.IrradPosition),
                ...
                dvc.meta_commit(
                    "<MASS_SPEC_FLUX_TRANSFER> "
                    "transferred mass spec fluxes for {}, {}".format(
                        irradiation, ",".join(state.levels)
                    )
                )
...
class MassSpecReducedNode(BaseMassSpecNode):
    ...
    def traits_view(self):
        v = okcancel_view(
            VGroup(
                VGroup(
                    UItem("message", style="custom"), label="Message", show_border=True
                ),
    ...
    def _make_production_file(self, repo, irrad, level):
        ...
        path = os.path.join(
            ...
            repo,
            "{}.{}.production.json".format(irrad, level),
        )
        dvc_dump(prod, path)
    ...
    def _save(self, repo):
        ...
        if dvc.repository_add_paths(repo, self._paths):
            dvc.repository_commit(
                repo, "<{}> {}".format(MASS_SPEC_REDUCED, self.message)
            )
