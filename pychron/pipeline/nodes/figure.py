...
from pychron.options.options_manager import (
    ...
    InverseIsochronOptionsManager,
    VerticalFluxOptionsManager,
    XYScatterOptionsManager,
    RadialOptionsManager,
    RegressionSeriesOptionsManager,
    FluxVisualizationOptionsManager,
    CompositeOptionsManager,
    RatioSeriesOptionsManager,
    RegressionOptionsManager,
)
        ...
        if use_plotting and self.use_plotting:
            for tab_id, unks in groupby_key(state.unknowns, "tab_id"):
                ...
                if state.correlation_ellipses:
                    editor.plotter_options._correlation_ellipses = (
                        state.correlation_ellipses
                    )
...
class VerticalFluxNode(FigureNode):
    name = "Vertical Flux"
    editor_klass = (
        "pychron.pipeline.plot.editors.vertical_flux_editor,VerticalFluxEditor"
    )
...
class FluxVisualizationNode(FigureNode):
    ...
    def run(self, state):
        ...
        if ps:
            ...
            editor.name = "Flux Visualization: {}{}".format(
...
class HistoryIdeogramNode(FigureNode):
    name = "Ideogram"
    editor_klass = (
        "pychron.pipeline.plot.editors.history_ideogram_editor,HistoryIdeogramEditor"
    )
    plotter_options_manager_klass = IdeogramOptionsManager
...
class RegressionSeriesNode(SeriesNode):
    ...
    editor_klass = (
        "pychron.pipeline.plot.editors.regression_series_editor,RegressionSeriesEditor"
    )
    ...
    def run(self, state):
        ...
        keys = [
            fi.name for fi in list(reversed([pi for pi in po.get_plotable_aux_plots()]))
        ]
