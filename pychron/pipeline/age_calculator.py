...
from pychron.processing.argon_calculations import calculate_f, age_equation, calculate_arar_decay_factors
...
class AgeCalculator(Loggable):
    ...
    def _calculate(self):
        ...
        # pi, ti, ti_p, _, _
        a, b = calculate_arar_decay_factors(dc37, dc39, [(1, 1, self.decay_days, None, None)])
        ...
        isos = (ufloat(getattr(self, attr), 0) for attr in ('ar40', 'ar39_corrected', 'ar38', 'ar37_corrected', 'ar36'))

        f, f_wo_irrad, non_ar_isotopes, computed, interference_corrected = calculate_f(isos, self.decay_days)
        uage = age_equation(ufloat(self.j, self.j_err), f)

        self.trait_set(age=float(nominal_value(uage)),
                       age_err=float(std_dev(uage)))

    def _calculate_button_fired(self):
        self._calculate()

    def traits_view(self):
        isogrp = BorderVGroup(Item('ar40'),
                              HGroup(Item('ar39'), Readonly('ar39_corrected')),
                              Item('ar38'),
                              HGroup(Item('ar37'), Readonly('ar37_corrected')),
                              Item('ar36'),
                              BorderVGroup(Item('correct_for_decay'),
                                           Item('decay_days'),
        ...
        v = View(VGroup(UItem('calculate_button'),
