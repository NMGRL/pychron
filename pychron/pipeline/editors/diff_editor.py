...
class DiffEditor(BaseTraitsEditor):
    ...
    def _find_right(self, left):
        ...
        return recaller.find_analysis(ln, aliquot, left.step)
    ...
    def _set_values(self, left, right, isotopes):
        ...
        if not self.is_blank and not self.is_air:
            ...
            vv = [
                Value(
                    name=n,
                    lvalue=nominal_value(getattr(constants, k)),
        ...
        for a in isotopes:
            ...
            vs.append(
                Value(
                    name=func("Filter Iter"),
                    lvalue=iso.filter_outliers_dict.get("iterations", 0),
            ...
            vs.append(
                Value(
                    name=func("Filter SD"),
                    lvalue=iso.filter_outliers_dict.get("std_devs", 0),
            ...
            vs.append(
                Value(
                    name=func("IC"),
                    lvalue=nominal_value(iso.ic_factor),
        ...
        for a in isotopes:
            ...
            if not (fv.lvalue == "no" and fv.rvalue == "no"):
                vs.append(
                    Value(
                        name=func("Bs Filter Iter"),
                        lvalue=baseline.filter_outliers_dict.get("iterations"),
                        rvalue=rbaseline.filter_outliers_dict.get("iterations"),
                    )
                )
                vs.append(
                    Value(
                        name=func("Bs Filter SD"),
                        lvalue=baseline.filter_outliers_dict.get("std_devs"),
        ...
        if not self.is_blank:
            for a in isotopes:
                ...
                vs.append(
                    Value(
                        name=func("Blank"),
                        lvalue=iso.blank.value,
                        rvalue=riso.blank.value,
                    )
                )
                vs.append(
                    Value(
    ...
    def _edit_configuration_button_fired(self):
        v = okcancel_view(
            VGroup(
                HGroup(UItem("select_all_button"), UItem("clear_all_button")),
                UItem(
                    "diff_tags",
                    style="custom",
        ...
        v = View(
            VGroup(
                ...
                HGroup(
                    ...
                    spring,
                    UItem("record_id", style="readonly"),
                    spring,
                ),
                UItem(
                    "values",
