...
from numpy import invert, zeros_like, asarray, max, copy, ones_like, zeros, uint8
...
class LumenDetector(Locator):
    ...
    def find_lum_peak(self, lum):
        self._mask(lum)
        h, w = lum.shape[:2]
        py, px = peak_local_max(rgb2gray(lum), min_distance=5, num_peaks=1)[0]

        c = circle(py, px, radius=4, shape=(h, w))
