...
class SimpleFurnace(StreamGraphManager):
    ...
    def _controller_default(self):
        c = NMGRLFurnaceController(name="controller", configuration_dir_name="furnace")
        return c
    ...
    def _update_scan_graph(self, response, output, setpoint):
        ...
        if update:
            ...
            if self._recording:
                if not self._last_update or time.time() - self._last_update > (
                    self.recording_period * 60
                ):
                    self._last_update = time.time()
                    self.record_data_manager.write_to_frame(
                        (x, response or 0, output or 0)
                    )
    ...
    def traits_view(self):
        v = View(
            VGroup(
                Item("setpoint"),
                Item("temperature_readback", style="readonly"),
                HGroup(
                    UItem("start_record_button"),
                    UItem("stop_record_button"),
