...
mgrp = BorderVGroup(
    ...
    HGroup(
        Spring(springy=False, width=100), Item("bottom_margin", label="Bottom"), spring
    ),
    label="Margins",
)
# cgrp = VGroup()
...
sgrp = BorderVGroup(
    ...
    HGroup(
        Item("use_column_width", enabled_when="not fit_to_page"),
    ...
    HGroup(
        Item(
            "fixed_width",
            label="W",
            enabled_when="not use_column_width and not fit_to_page or "
            'page_type=="custom"',
        ),
        Item(
            "fixed_height",
            label="H",
            visible_when='not fit_to_page or page_type=="custom"',
        ),
...
agrp = BorderHGroup(
    Item("valign", label="Vertical"), Item("halign", label="Horizontal"), label="Align"
)

PDFLayoutGroup = VGroup(
    Item("orientation"),
    agrp,
    mgrp,
    sgrp,
...
PDFLayoutView = okcancel_view(PDFLayoutGroup, title="PDF Save Options")
