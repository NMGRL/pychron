...
from pychron.igsn.definitions import (
    SAMPLE_ATTRS,
    SAMPLE_TYPES,
    ROCK_TYPES,
    SUB_ROCK_TYPES,
...
class IGSNSampleModel(HasTraits):
    ...
    def traits_view(self):
        required_grp = VGroup(
            Item("sample_type", label="Sample Type"),
        ...
        collection_grp = VGroup(
            Item("collector", label="Collection"),
            Item("collection_method", label="Collection Method"),
            HGroup(
                Item("_collection_start_date", label="Collection Start Date"),
        ...
        location_grp = VGroup(
            Item("latitude", label="Latitude", tooltip=LAT_TT),
        ...
        optional_grp = VGroup(
            VGroup(
                UItem("description", style="custom", height=-100),
                label="Description",
                show_border=True,
            ),
        ...
        classification_grp = VGroup(
            Item("rock_type", label="Rock Type"),
            Item(
                "sub_rock_type",
        ...
        v = View(
            Tabbed(
                VGroup(
                    Item("user_code", label="SESAR User Code", style="readonly"),
                    required_grp,
                    classification_grp,
                    optional_grp,
                    label="New Sample",
...
class IGSNService(Loggable):
    ...
    def get_new_igsn(self, sample="", material=""):
        ...
        r = self._post(locator, data={"content": content})
