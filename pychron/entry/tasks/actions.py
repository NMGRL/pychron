...
class AddFluxMonitorAction(Action):
    ...
    def perform(self, event):
        ...
        s = app.get_service(
            "pychron.entry.editors.flux_monitor_editor.FluxMonitorEditor"
        )
        s.add_flux_monitor()
...
class GenerateIdentifiersAction(TaskAction):
    ...
    description = (
        "Automatically generate labnumbers (aka identifiers) for each irradiation position in the "
...
class CorrelationEllipsesAction(Action):
    ...
    def perform(self, event):
        ...
        if dvc is not None:
            from pychron.entry.correlation_ellipses_editor import (
                CorrelationEllipsesEditor,
            )
...
class GenerateTrayAction(Action):
    ...
    def perform(self, event):
        ...
        dlg = FileDialog(
            action="open",
            default_directory=os.path.join(paths.meta_root, "irradiation_holders"),
        )
        if dlg.open() == OK:
            p = dlg.path
        ...
        if p is not None:
            ...
            bounds = (2.54, 2.54)
            radius = 0.03 * 2.54
            gcc, gm = open_txt(p, bounds, radius, convert_mm=True, make=True, rotate=0)
...
class ImportIrradiationFileAction(TaskAction):
    ...
    description = (
        'Import irradiation information from an Excel file. Use "Irradiation Template" '
...
class MakeIrradiationTemplateAction(Action):
    ...
    def perform(self, event):
        ...
        dialog = FileDialog(
            action="save as", default_filename="IrradiationTemplate.xls"
        )
        ...
        if dialog.open() == OK:
            ...
            if path:
                ...
                from pychron.entry.loaders.irradiation_template import (
                    IrradiationTemplate,
                )
