...
class SamplesAdapter(TabularAdapter):
    columns = [("Name", "name"), ("Material", "material"), ("Grainsize", "grainsize")]
    font = "arial 10"
    odd_bg_color = "#d6f5f5"
...
class SamplePrepPane(TraitsTaskPane):
    def traits_view(self):
        hgrp = VGroup(
            UItem(
                "object.active_sample.steps",
                editor=TabularEditor(
                    adapter=PrepStepAdapter(),
...
class SamplePrepSessionPane(TraitsDockPane):
    ...
    def traits_view(self):
        wgrp = HGroup(
            UItem("worker", editor=EnumEditor(name="workers")),
            icon_button_editor("add_worker_button", "add", tooltip="Add a new worker"),
            show_border=True,
            label="Worker",
        )
        sgrp = HGroup(
            UItem("session", editor=EnumEditor(name="sessions")),
            icon_button_editor(
                "add_session_button", "add", tooltip="Add a new session for this worker"
            ),
            icon_button_editor(
                "edit_session_button",
...
class SamplePrepFilterPane(TraitsDockPane):
    ...
    def traits_view(self):
        ...
        pgrp = VGroup(
            UItem("project", editor=EnumEditor(name="projects")),
            label="Project",
            show_border=True,
        )
        sgrp = VGroup(
