...
from traitsui.api import View, Item, VGroup, CheckListEditor
...
class UserEntry(IsotopeDatabaseManager):
    user = Str
    email = Str
    categories = List
    ...
    def _edit_user(self, dbuser):
        ...
        # self.information_dialog('Editing user not currently implemented')
        # self.original_user=user
        #db=self.db
        #with db.session_ctx():
        #    dbproj=db.get_project(proj)
        # self.user_style='readonly'
    ...
    #
    #              kind='modal',
    #              resizable=True, title='Edit User',
    #              buttons=['OK', 'Cancel'])
    #     return v
