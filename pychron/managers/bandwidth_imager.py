    ...
    def traits_view(self):
        v = View(
            HGroup(
                ...
                Item("center"),
                Item("threshold"),
                Item("color", style="custom", show_label=False),
            )
        )
        return v
...
class BandwidthImager(HasTraits):
    ...
    def _calc_area(self):
        self.trait_set(
            low=self.calc_area_value - self.calc_area_threshold,
    ...
    def render_pdf(self, obj, path):
        ...
        gc.gc.drawString(
            600,
            5,
            "area:{:0.3f}% low={} high={}".format(self.area, self.low, self.high),
        )
    ...
    def _refresh(self):
        if self.use_threshold:
            ...
            self.area = (sum(tim) / (ndim.shape[0] * ndim.shape[1])) / 255.0
