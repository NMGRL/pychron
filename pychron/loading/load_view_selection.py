...
from traits.api import HasTraits, Instance, DelegatesTo, Button, String
...
class LoadViewSelectionModel(HasTraits):
    ...
    def _selection_str_changed(self, new):
        if new:
            fs = [(ps.labnumber, pp) for ps in self.positions for pp in ps.positions]
    ...
    def _select_all_button_fired(self):
        self.selected_positions = [
            LoadPosition(labnumber=ps.labnumber, positions=[pp])
...
class PositionsAdapter(TabularAdapter):
    columns = [("Identifier", "labnumber"), ("Position", "position_str")]
...
class LoadViewSelectionController(Controller):
    ...
    def traits_view(self):
        grp = VGroup(
            UItem("load_name", editor=EnumEditor(name="loads")),
            HGroup(UItem("selection_str"), UItem("select_all_button")),
            HGroup(
                UItem("canvas", editor=ComponentEditor(), style="custom"),
                UItem(
                    "selected_positions",
