...
class IntensitiesPane(TraitsDockPane):
    ...
    def traits_view(self):
        cols = [
            ColorColumn(cell_color_name="color", label="Color"),
        ...
        grp = VGroup(
            ...
            VGroup(
                HGroup(
                    ...
                    Item(
                        "object.graph.marker_tool.label_with_intensity",
                        tooltip="Label marker with isotopic intensity",
                        label="Intensity",
                    ),
                ),
                UItem(
                    "object.graph.markers",
                    editor=TableEditor(
                        columns=cols,
                        selection_mode="rows",
                        sortable=False,
...
class MassScannerPane(TraitsDockPane):
    ...
    def traits_view(self):
        grp = VGroup(
            ...
            HGroup(
                Item("scanner.start_mass", format_str="%0.3f"),
...
class DACScannerPane(TraitsDockPane):
    ...
    def traits_view(self):
        grp = VGroup(
            ...
            HGroup(
                Item("scanner.step", format_str="%0.5f"),
                UItem("scanner.scan_time_length"),
            ),
            HGroup(
                Item("scanner.min_dac", label="Min", format_str="%0.5f"),
                Item("scanner.max_dac", label="Max", format_str="%0.5f"),
                icon_button_editor(
                    "scanner.use_mftable_limits",
                    "foo",
            ...
            HGroup(
                Item("scanner.scan_min_dac", label="Min", format_str="%0.5f"),
...
class ControlsPane(TraitsDockPane):
    ...
    def traits_view(self):
        ...
        detector_grp = VGroup(
            HGroup(spring, Label("Deflection"), Spring(springy=False, width=70)),
            Item(
                "detectors",
                show_label=False,
                editor=ListEditor(
