...
class KerrStepMotor(KerrMotor):
    ...
    def load_additional_args(self, config):
        ...
        for section, option in [
            ("Parameters", "run_current"),
            ("Parameters", "hold_current"),
        ]:
            self.set_attribute(
                config, option, section, option, optional=False, cast="int"
            )

        self.set_attribute(
            config, "speed_mode", "Parameters", "speed_mode", optional=False
        )
        self.set_attribute(
            config,
            "disable_estop",
    ...
    def control_view(self):
        v = View(
            ...
            Item(
                "data_position",
                show_label=False,
                editor=RangeEditor(mode="slider", low_name="min", high_name="max"),
            ),
            Item(
                "update_position",
                show_label=False,
                editor=RangeEditor(
                    mode="slider", low_name="min", high_name="max", enabled=False
                ),
            ),
        )
        return v
