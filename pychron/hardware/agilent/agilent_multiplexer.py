...
class Channel(HasTraits):
    ...
    def traits_view(self):
        v = View(
            HGroup(
                Item("name", show_label=False, style="readonly", width=200),
    ...
    def load_additional_args(self, config):
        ...
        for section in config.sections():
            if section.startswith("Channel"):
                ...
                threshold = self.config_get(
                    config, section, "threshold", cast="float", default=None
                )
                if threshold is not None:
                    inv = self.config_get(
                        config, section, "inverted_logic", cast="boolean", default=False
                    )
                ...
                ch = Channel(
                    address="{}{:02d}".format(self.slot, int(section[7:])),
                    kind=kind,
                    name=name,
                    equation=eq,
                )
