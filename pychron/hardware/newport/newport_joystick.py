...
class Joystick(HasTraits):
    ...
    def disable(self):
        """ """
        com = ";".join(
            [self.parent._build_command_("BQ", xx=a.id, nn=0) for a in self.parent.axes]
        )[:-1]
        self.parent.ask(com)
        com = ";".join(
            [self.parent._build_command_("TJ", xx=a.id, nn=1) for a in self.parent.axes]
        )[:-1]
        self.parent.ask(com)
    ...
    def traits_view(self):
        ...
        return View(dio_group, resizable=True)
