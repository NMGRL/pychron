...
class YamlObject(HasTraits):
    ...
    def dump(self):
        ...
        return {k: get(k) for k in self.trait_names() if not k in excludes}
...
class MeasurementContextEditor(ContextEditor):
    ...
    def dump(self):
        ctx = dict(
            default_fits=self.default_fits,
    ...
    @on_trait_change(
        "peakhop:+, multicollect:+, baseline:+, peakcenter:+, equilibration:+, default_fits"
    )
    ...
    def _get_available_hops(self):
        return glob_list_directory(
            os.path.join(paths.measurement_dir, "hops"),
            extension=".txt",
    ...
    def traits_view(self):
        mc_grp = VGroup(
            HGroup(
                Item(
                    "object.multicollect.isotope",
                    editor=ComboboxEditor(name="isotopes"),
                ),
                Item(
                    "object.multicollect.detector",
        ...
        bs_grp = VGroup(
            HGroup(
                ...
                Item(
                    "object.baseline.detector",
                    editor=ComboboxEditor(name="available_detectors"),
                ),
                Item("object.baseline.counts"),
            ),
            HGroup(Item("object.baseline.before"), Item("object.baseline.after")),
            Item(
                "object.baseline.settling_time",
            ),
            show_border=True,
            label="Baseline",
        )
        ...
        pc_grp = VGroup(
            HGroup(
                Item(
                    "object.peakcenter.isotope",
                    editor=ComboboxEditor(name="isotopes"),
                    label="Iso.",
                ),
                Item(
                    "object.peakcenter.detector",
                    label="Det.",
                    editor=ComboboxEditor(name="available_detectors"),
                ),
            ),
            Item("object.peakcenter.integration_time"),
            HGroup(Item("object.peakcenter.before"), Item("object.peakcenter.after")),
            Item(
                "object.peakcenter.detectors",
                style="custom",
        ...
        gen_grp = VGroup(
            Item("default_fits", editor=EnumEditor(name="available_default_fits")),
            show_border=True,
            label="General",
        )
