...
class ExtractionLineScriptWriter(Loggable):
    ...
    def save_as(self):
        dlg = FileDialog(action="save as", default_directory=paths.extraction_dir)
        if dlg.open() == OK:
            p = dlg.path
            if p:
                self._save(p)
    ...
    def _open_file(self, p):
        ...
        self._path = p
        valve_re = re.compile(
            r"""(?P<action>(open|close)+)\((name *= *)*['"]{1}(?P<name>[\w\d])+['"]{1}\,*[\w\s]*\)"""
        )
...
{}""".format(
    ...
    def _canvas_default(self):
        elc = ExtractionLineCanvas2D(manager=self, confirm_open=False)
